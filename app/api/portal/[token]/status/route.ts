...
// Branding rides along here too — it's needed on every portal screen
// (including the pre-MFA gate), so a second round-trip isn't worth it.
export const GET = withErrorHandling(async function GET(req: Request, { params }: { params: { token: string } }) {
  ...
  return NextResponse.json({ valid: true, mfaCurrent: ctx.mfaCurrent, branding });
});
