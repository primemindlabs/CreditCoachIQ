...
## Module P — DisputeFox-parity Phase 1 (leads, two-way SMS, bulk letters, dialer follow-up)

See `DISPUTEFOX_PARITY_SCOPE.md` for the full 8-item competitive scope this closes the P1 cluster of. Two items in that scope (SMS and the dialer) turned out to be much smaller than first estimated once the actual codebase was checked — both already had working backends, just no UI.

- ✅ **Leads pipeline**: no separate `leads` table — a lead is a `borrowers` row with `lead_status != 'converted'` (migration 0013), since `borrowers` already exists independently of `credit_repair_enrollments`. `app/api/leads` (list, manual create — deliberately does not touch Stripe/enrollment), `app/api/leads/[id]` (status/interest/note updates, logged to append-only `lead_activity_log`), `app/api/leads/[id]/convert` (the explicit action that creates the Stripe customer + trial enrollment, same CROA state-gate as `/api/enroll`). `app/(dashboard)/leads` list UI; `/caseload/[borrowerId]` doubles as the lead detail page (it already conditionally hides enrollment-only sections) with a "Convert to client" button when there's no enrollment yet. `/api/coach/caseload` now filters to `lead_status = 'converted'` so leads don't mix into the client list.
- ✅ **Two-way SMS**: `sms_messages` table + `app/api/webhooks/twilio/inbound` (signature-verified inbound webhook, resolves the borrower by matching the last 10 digits of the sender's number since there's one shared `TWILIO_FROM_NUMBER` for the whole install) + `app/api/coach/sms` (thread GET/POST, marks inbound read on open) + `lib/sms.ts`'s new `sendSms()` helper. Thread panel on `/caseload/[borrowerId]`; unread texts roll into the Today page's "Unread messages" stat and list alongside portal messages.
...
- **SmartCredit API integration** — confirmed directly against EquityNest's own ConsumerDirect PartnerHub account (partnerhub.consumerdirect.com), not just public docs: the partner program is a referral/subscription reseller relationship, not a data API. Its "Client Manager" feature (subscriber list — status, name, email, phone, plan, price, CSV export) is billing/subscriber management, not credit report or tradeline data. There is no report-pulling capability anywhere in the real partner account. What exists instead: (1) the affiliate referral link (already built), and (2) once PartnerHub onboarding finishes, the Client Manager CSV could drive a smaller *subscriber-status* sync (mark a lead as "active SmartCredit subscriber") — genuinely buildable later, but that is not report data. Actual report import stays the AI-parsed PDF upload flow (`/credit-reports`, already built) — client or coach exports the PDF from their own SmartCredit account and uploads it. Revisit only if EquityNest negotiates a data-specific agreement with ConsumerDirect beyond the standard partner program.
