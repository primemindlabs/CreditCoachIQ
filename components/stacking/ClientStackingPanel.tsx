...
// Scoped to one client. Business credit profiles, funding plans, and lender
// applications, gated to the Investor Path plan tier. Data capture and
// tracking only, no AI stack-recommendation engine (that decision is made
// with Trulli AI outside this system).
export default function ClientStackingPanel({ borrowerId, planTier }: { borrowerId: string; planTier: string }) {
  ...
  useEffect(() => {
    if (planTier === 'investor_path') loadDetail();
  }, [planTier, loadDetail]);

  async function saveProfile() {
    if (!profileForm.entityName.trim()) return;
  ...
  async function revealEin(profileId: string) {
    ...
    const res = await fetch(`/api/stacking/business-profiles?borrower_id=${borrowerId}&reveal=true`);
    if (!res.ok) return;
  ...
  async function generatePlan() {
    ...
    try {
      ...
      if (planInputMethod === 'upload_new' && planNewDocFile) {
        ...
        const upRes = await fetch(`/api/coach/client/${borrowerId}/documents`, { method: 'POST', body: form });
        if (!upRes.ok) {
      ...
      const res = await fetch('/api/stacking/plans', {
        ...
        body: JSON.stringify({
          ...
          planned_sequence: docNote ? [{ note: docNote }] : [],
        }),
      });
      const d = await res.json().catch(() => ({}));
      if (!res.ok) {
      ...
      if (planInputMethod === 'upload_new') {
        const docsRes = await fetch(`/api/coach/client/${borrowerId}/documents`).then((r) => (r.ok ? r.json() : { documents: [] }));
        setVaultDocs(docsRes.documents ?? []);
      }
    } catch {
  ...
  async function addApplication(planId: string) {
    ...
    try {
      const res = await fetch('/api/stacking/applications', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ stack_plan_id: planId, borrower_id: borrowerId, lender_name: form.lenderName.trim(), product_name: form.productName || null }),
      });
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
  ...
  if (planTier !== 'investor_path') {
    return (
      <div className="rounded-card border border-line bg-gold-tint p-6 text-sm text-ink">
        Credit stacking requires the Investor Path plan tier. Upgrade this client&apos;s plan tier above to unlock it.
      </div>
    );
  }
    ...
    <div>
      {summary && (
        <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <StatCard
            label="Promo expiring under 30d"
      ...
      <div className="mb-6 rounded-card border border-line bg-white p-6 shadow-card">
        ...
        <div className="mb-4 rounded-control border border-line bg-paper p-3 text-xs text-muted">
          Recommended: use a tri-merge report (all 3 bureaus) as the basis for this plan. Single-bureau reports are supported but tend to surface fewer usable tradelines.
        </div>
